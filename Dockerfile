...
RUN apt-get update && apt-get install -y dovecot-core dovecot-pgsql dovecot-sieve dovecot-managesieved dovecot-imapd dovecot-pop3d dovecot-lmtpd dovecot-ldap dovecot-solr rspamd && apt-get clean
...
RUN chown -R 1000:1000 /var/lib/dovecot
